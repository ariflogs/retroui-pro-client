    ...
    <section className="min-h-screen bg-[#599D77] flex flex-col items-center justify-center py-20 px-4">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center mb-16">
          ...
          <Text className="text-base lg:text-lg text-white/80 font-medium max-w-2xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center items-center gap-8">
          {data.map((member) => (
            <Card
              ...
              onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && setSelectedMember(member.id)}
              className={`relative h-[500px] shadow-lg hover:shadow-lg border-3 ${selectedMember === member.id ? 'w-[500px]' : 'w-[280px]'} overflow-hidden duration-400 cursor-pointer group rounded-2xl`}
            >
              <img
                src={member.decorImage}
                alt=""
                className={`absolute object-cover transition-all duration-400 ease-out ${
                  selectedMember === member.id
                    ? 'top-[calc(100%-11.5rem)] -left-10 w-56 h-56 rotate-180'
                    : '-top-10 left-[calc(100%-8.5rem)] w-44 h-44 rotate-0'
                }`}
              />
              <img src={member.profileImage} alt={member.name} className={`h-2/3 object-cover absolute right-0 bottom-0 transition-all duration-400 ${selectedMember === member.id ? 'h-[400px] -right-16' : ''}`} />
              <div className={`flex flex-col justify-start items-start p-6 lg:p-8 transition-opacity duration-400 opacity-0 pointer-events-none ${selectedMember === member.id ? 'delay-300 opacity-100' : ''}`}>
                <Text as="h3" className="font-normal text-4xl lg:text-5xl mb-4">{member.name}</Text>
