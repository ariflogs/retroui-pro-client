      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.map((member) => (
            <div
              key={member.id}
              className="overflow-hidden group relative min-h-[600px] border-3 shadow-lg hover:shadow-lg bg-cover bg-center"
              style={{ backgroundImage: `url(${member.image})` }}
            >
              {/* Info Section */}
              <Card className="shadow-primary shadow-md hover:shadow-md group-hover:shadow-xs absolute bottom-6 left-6 right-6">
                <Card.Header>
                    ...
                    {socialLinks.map(({ icon: Icon, label, key }) => {
                      ...
                      return (
                        <a
                          key={key}
                          href={href}
                          className="size-9 bg-background border-2 flex items-center justify-center hover:bg-primary transition-colors"
                          aria-label={label}
                        >
...
const socialLinks = [
  ...
  { icon: MailIcon, label: "Email", key: "email" },
] as const;
...
const data = [
  ...
  },
  {
    ...
    bio: "An eager learner and creative developer building real-time apps.",
  ...
  },
  {
    ...
    bio: "A curious thinker and skilled coder creating innovative solutions.",
  ...
  },
  {
    ...
    bio: "A creative mind and skilled coder turning ideas into reality.",
